...
App/HandCamera.swift uses AVCaptureDevice.RotationCoordinator for camera-specific buffer rotation (a fixed 90-degree angle produced a sideways preview on the iPhone 17 Pro). It captures mirrored front-camera frames and extracts Apple Vision hand landmarks at up to 15 Hz. App/HandControl.swift owns a pure, testable gesture gate: open hand after enable/loss, thumb/index pinch held 150 ms to move, broad lateral zones with hysteresis to steer, immediate zero on release/invalid hand. Wrist-to-middle-base normalises the pinch; other finger visibility is not required. Brief tracking loss can resume only after a fresh dwell; loss over 600 ms requires opening again. Left/right turns stop the inside wheel and drive the outside wheel symmetrically. Hand mode is forward-only, capped at 35% power. BluetoothLink independently disarms if camera callbacks stall for 300 ms. App/GestureGuide.swift renders procedural SceneKit 3D animations for forward/left/right/stop. The toolbar hand icon always opens it, stops the rover and pauses the camera. Closing it requires rearming. Camera frames are processed locally without recording or upload.
